<template>
  <div>
    ...
    <q-list v-else bordered class="rounded-borders bg-white" separator square>
      <q-item class="bg-primary text-white text-left">
        <!-- <q-item-section style="max-width: 20px;">No.</q-item-section> -->
        <q-item-section>Nama</q-item-section>
        <q-item-section>NIK</q-item-section>
        <q-item-section>Tanggal Lahir</q-item-section>
        <q-item-section>Agama</q-item-section>
        <q-item-section side>
          #
        </q-item-section>
      </q-item>
      <q-item v-for="(row, i) in props?.listmaster" :key="row?.no || i" v-ripple class="q-py-xs">
        ...
        <q-item-section>{{ row.nik }}</q-item-section>
        <q-item-section>{{ row.tgl_lahir }}</q-item-section>
        <q-item-section>{{ row.agama }}</q-item-section>
        <q-item-section side>
